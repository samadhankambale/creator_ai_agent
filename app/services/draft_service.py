...
class DraftService:
    ...
    def get_active_draft(self, db: Session, user_id: str) -> Draft | None:
        """Get the user's current active draft (updated within 2 hours)."""
        from datetime import datetime, timedelta
        cutoff = datetime.utcnow() - timedelta(hours=2)
        # Auto-discard stale drafts so they don't block new posts
    ...
    def update_extracted_data(
        ...
        flag_modified(draft, "extracted_data")  # force SQLAlchemy to detect JSON change
        draft.updated_at = datetime.utcnow()
        db.commit()
        db.refresh(draft)
        return draft
    ...
    def update_generated_assets(
        ...
        flag_modified(draft, "generated_assets")  # force SQLAlchemy to detect JSON change
        draft.updated_at = datetime.utcnow()
        db.commit()
        db.refresh(draft)
        return draft
